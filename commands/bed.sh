...
bed.mesh(){
	...
	_get /printer/objects/query 'bed_mesh' > bed_mesh.tmp.json
	mesh_min=$(jq '.result.status.bed_mesh.mesh_matrix | flatten(1) | min' bed_mesh.tmp.json)
	mesh_max=$(jq '.result.status.bed_mesh.mesh_matrix | flatten(1) | max' bed_mesh.tmp.json)
