...
END {
    ...
    #print "positive_gradient_scale_spacing",positive_gradient_scale_spacing
    #print "negative_gradient_scale_spacing",negative_gradient_scale_spacing
    #negative_gradient_scale_spacing = -0.00396092
    ...
    for (row in results){
        ...
        for (col in results[row]){
            ...
            if ( col == 1 && (row %2 == 0 || row == length(results)) ){
                printf("%s", go_to_col(35));
                printf("%s%2s\033[0m", colors["xy"], to_superset(row_idx_display));
                printf("%s", go_to_col(grid_start_indent+35));
                row_idx_display++;
            }
        ...
        # Every other line, print the saved data for this line and the previous line (compiled into one line)
        if ( row % 2 == 0  ) {
            ...
            printf("%s", go_to_col(67));
            if ( row_idx_display == 1 ){
                # Very first gradient row at top
            ...
            printf("%s", go_to_col(67));
            #printf("\033[38;2;%sm%s%s\033[0m %s %s%s\033[38;5;0m\n", mesh_val_to_color(results[row][col]),  block["upper"], block["upper"],  to_superset(trim_gradient(min_value)), colors["xy"], to_superset("low"));
    ...
    printf("%s%s%-3s %s %3s%s\n", go_to_col(45), colors["coordinates"], coordinates["left"], coordinates["X"], coordinates["right"], "\033[0m");
}